<template>
  <section class="flex flex-col items-center gap-6">
    <img src="/lain.gif" class="w-64 h-64 rounded-full" />
    ...
    <form @submit.prevent="handleSubmit" class="flex flex-col gap-2 w-full">
      ...
      />
      <!-- <small class="text-slate-500 italic"> Please provide a name </small> -->
      <button
        :disabled="hasNameError"
        type="submit"
        class="transition w-full bg-purple-950 text-slate-300 font-medium py-2 px-3 rounded hover:bg-opacity-90"
      >
        Start a New Chat
      </button>
      <button
        type="button"
        class="transition w-full bg-purple-950 text-slate-200 font-medium py-2 px-3 rounded hover:bg-opacity-90"
      >
